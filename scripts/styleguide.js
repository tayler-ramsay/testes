...
var StyleguideIndex = {
  init: function() {
    if ($('.pavone-iframe-content').length && !FRONT_END_TEST) return false;

    this.$body = $('body');
    this.$breakpointsLinks = $('.pavone-header__breakpoints__item__link');
    this.$sidebarContent = $('.pavone-sidebar__content');
    this.$sidebarLinks = $('.pavone-sidebar__nav__item__link');
    this.$sidebarToggle = $('[class*="pavone-sidebar__toggle"]');
    this.$iframe = $('.pavone-iframe-wrapper iframe');
    this.$iframeContent = this.isChromeAndFileProtocol() ? null : $('.pavone-iframe-wrapper iframe').contents();
